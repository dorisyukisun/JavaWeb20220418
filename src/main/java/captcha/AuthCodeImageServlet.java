...
//1.自動產出 0000-99995 之間的認證碼 (含干擾圖像)
...
public class AuthCodeImageServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//將驗證碼保留在session 中以便日後驗證
		req.getSession().setAttribute("authCode", authCode);
		
		//產生驗證碼圖片
		try {
			ImageIO.write(getAutherCodeImage(authCode),"JPEG", resp.getOutputStream());
		  }catch (Exception e) {
			e.printStackTrace();
		}
	...
	//產生驗證碼圖片
	private BufferedImage getAutherCodeImage(String authCode) {
		//建立圖像暫存區
