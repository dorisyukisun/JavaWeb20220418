...
public class JPAService {
	...
	public List<Guestbook> queryGuestbook(){
	  ...
	  // JPQL 所指的Guestbook 是entity 的名字而非table name guestbook
	  // 所以本題 select g Guestbook g 成功
	 // 所以本題 select g guestbook g 失敗
	...
	{
		synchronized (em) {
			//if(getGuestbookById(guestbook.getId()) !=null) {//如果有資料的話 就可以修改
				EntityTransaction etx = em.getTransaction();
				etx.begin();//開始
				//使用 merge 若該筆不存在會新增,若該筆存在則修改
				//使用 merge 可以在unbind 的情況下使用
