...
public class GetGuestbook extends HttpServlet{
	...
	private void doHandle(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 req.setAttribute("button_name","update");// 改成update
		 req.setAttribute("guestbook", guestbook);//傳給jsp 並放到 form表單裡使用
		 /*req.setAttribute("id",guestbook.getId());//傳給jsp 並放到 form表單裡使用
		 req.setAttribute("username", guestbook.getUsername());//傳給jsp 並放到 form表單裡使用
		 req.setAttribute("content", guestbook.getContent()); // 傳給 jsp 並放到 form 表單中使/
		 */
